...
A dockerizaton of the pigeoncoin wallet and associated ccminer/nevermore miner with x16s support.

# Wallet

On any docker-engine, you may run the wallet:

    docker-compose up -d pigeoncoin-wallet

Once running, you can docker exec into it and run `pigeon-cli` to interact with the wallet:

    docker exec -ti pigeoncoin-wallet bash
    $ pigeon-cli getnewaddress ""
    $ pigeon-cli getaddressesbyaccount ""
    $ pigeon-cli getbalance ""

# Mining

The `pigeoncoin-gpu-ccminer` container requires a docker-engine with the nvidia runtime enabled by default.
...
Start the miner using this:
    ...
    docker-compose up -d pigeoncoin-gpu-ccminer

Watch the logs using this:

    docker-compose logs -f --tail=100 pigeoncoin-gpu-ccminer
